...
class PayMongoService {
  ...
  static Future<Map<String, dynamic>> createPaymentIntent({
    ...
    try {
      ...
      return {'success': false, 'error': 'Payment intent creation failed: $e'};
    }
  }
  ...
  static Future<Map<String, dynamic>> createWebhook({
    ...
    try {
      final response = await http.post(
        ...
        body: jsonEncode({
          'data': {
            'attributes': {'url': url, 'events': events},
          },
        }),
      );
  ...
  static Future<Map<String, dynamic>> createPaymentMethod({
    ...
    try {
      ...
      return {'success': false, 'error': 'Payment method creation failed: $e'};
    }
  }
  ...
  static Future<Map<String, dynamic>> attachPaymentMethod({
    ...
    try {
      ...
      return {'success': false, 'error': 'Attach payment method failed: $e'};
    }
  }
  ...
  static Future<Map<String, dynamic>> retrievePaymentLink(String linkId) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        bool isPaid =
            payments?.any((p) => p['attributes']?['status'] == 'paid') ?? false;

        return {'success': true, 'isPaid': isPaid, 'data': data};
      } else {
  ...
  static Future<Map<String, dynamic>> createPaymentLink({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final checkoutUrl =
            responseData['data']?['attributes']?['checkout_url'] ??
  ...
  static Future<Map<String, dynamic>> createGCashPaymentLink({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to create GCash payment link: ${response.body}',
        );
      }
    } catch (e) {
